...
# K3s + Rancher Kurulum Script'i
# Bu script K3s üzerine Rancher'ı kurar
...
# K3s kurulumu
install_k3s() {
    log_info "K3s kuruluyor..."
    
    if ! command -v k3s &> /dev/null; then
        # K3s'i kur
        curl -sfL https://get.k3s.io | sh -
        
        # K3s servisini başlat
        sudo systemctl start k3s
        sudo systemctl enable k3s
        
        log_success "K3s kuruldu ve başlatıldı"
    else
        log_info "K3s zaten kurulu: $(k3s --version)"
    fi
}

# K3s kubeconfig'ini ayarla
setup_kubeconfig() {
    log_info "K3s kubeconfig ayarlanıyor..."
    ...
    if kubectl get nodes &> /dev/null; then
        ...
        log_error "kubectl çalışmıyor. K3s kurulumunu kontrol edin."
        exit 1
    fi
}
...
show_access_info() {
    log_info "=== K3S + RANCHER ERİŞİM BİLGİLERİ ==="
    ...
    echo
    echo "=== K3s Yönetimi ==="
    echo "K3s durumu: sudo systemctl status k3s"
    echo "K3s başlat: sudo systemctl start k3s"
    echo "K3s durdur: sudo systemctl stop k3s"
    echo "K3s yeniden başlat: sudo systemctl restart k3s"
    echo
    ...
    echo
    echo "=== K3s Avantajları ==="
    echo "✓ Daha hafif (RKE2'den %60 daha az kaynak)"
    echo "✓ Daha hızlı başlatma"
    echo "✓ SQLite veritabanı (etcd yerine)"
    echo "✓ Tek binary kurulum"
}

# Ana fonksiyon
main() {
    log_info "=== K3S + RANCHER KURULUMU BAŞLATILIYOR ==="
    echo
    ...
    log_success "=== K3S + RANCHER BAŞARIYLA KURULDU! ==="
    echo
    ...
    echo
    log_info "K3s avantajları:"
    log_info "• Daha hafif ve hızlı"
    log_info "• Daha az kaynak kullanımı"
    log_info "• Kolay kurulum ve yönetim"
}
